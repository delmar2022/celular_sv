...
$resultado = mysqli_query($conexion,"SELECT id_cliente, nombre_cliente, fiscal_cliente, telefono_cliente, email_cliente, tipo_cliente FROM clientes WHERE nombre_cliente LIKE '%" . $q . "%' OR fiscal_cliente LIKE '%" . $q . "%'");
...
while($row = mysqli_fetch_array($resultado))
    {
      if ($row['tipo_cliente'] == 1) {
        ...
        $pdf->Cell($w[0],6,utf8_decode($row['id_cliente']),1, 0 , 'L', $bandera);
        $pdf->Cell($w[1],6,utf8_decode($row['nombre_cliente']),1, 0 , 'L', $bandera);
        $pdf->Cell($w[2],6,utf8_decode($row['fiscal_cliente']),1, 0 , 'L', $bandera);
        $pdf->Cell($w[3],6,utf8_decode($row['telefono_cliente']),1, 0 , 'L', $bandera);
        $pdf->Cell($w[4],6,utf8_decode($row['email_cliente']),1,0 , 'L', $bandera);
