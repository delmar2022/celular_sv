...
if (!empty($id) and !empty($cantidad)) {
    ...
    $id_comp        = get_row('facturas_ventas', 'id_comp_factura', 'id_factura', $id_factura);
    ...
    if ($row = mysqli_fetch_array($comprobar)) {
        ...
        if ($cant > $stock and $inv == 0) {
            ...
            if ($id_comp = 3) {
                echo '<script>
        $("#carga_totales").load("../ajax/carga_total_tmp7.php");
         </script>';
            } else {
                echo '<script>
        $("#carga_totales").load("../ajax/carga_total_tmp8.php");
         </script>';
            }
        }
...
while ($row = mysqli_fetch_array($sql)) {
    ...
    //$id_com          = get_row('facturas_ventas', 'id_comp_factura', 'id_factura', $id_factura);
    $precio_venta_f  = formato($precio_venta); //Formateo variables
    $precio_venta_r  = str_replace(",", "", $precio_venta_f); //Reemplazo las comas
    $precio_total    = $precio_venta_r * $cantidad;
    $final_items     = rebajas($precio_total, $desc_tmp); //Aplicando el descuento
    /*--------------------------------------------------------------------------------*/
    $precio_total_f = formato($final_items); //Precio total formateado
    $precio_total_r = str_replace(",", "", $precio_total_f); //Reemplazo las comas
    $sumador_total += $precio_total_r; //Sumador
    $subtotal = formato($sumador_total);
    if ($row['iva_producto'] == 1) {
        $total_iva = iva($precio_venta);
    } else {
        $total_iva = 0;
    }
    //$ret = get_row('clientes', 'ivaret_cliente', 'id_cliente', $cliente_id);
    $total_impuesto += rebajas($total_iva, $desc_tmp) * $cantidad;
    $importe   = $sumador_total / 1.13;
    $tot       = $tot + $importe;
    $iva       = $importe * 0.13;
    $subtot    = $sumador_total + $total_impuesto;
    $total     = $subtot;
...
$sql1 = mysqli_query($conexion, "select * from productos where id_producto='" . $id_producto . "'");
    while ($rw1 = mysqli_fetch_array($sql1)) {
        ?>
                        <option selected disabled value="<?php echo $precio_venta ?>"><?php echo number_format($precio_venta, 4); ?></option>
                        <option value="<?php echo $rw1['valor1_producto'] ?>">PV <?php echo number_format($rw1['valor1_producto'], 4); ?></option>
                        <option value="<?php echo $rw1['valor2_producto'] ?>">PM <?php echo number_format($rw1['valor2_producto'], 4); ?></option>
                        <option value="<?php echo $rw1['valor3_producto'] ?>">PE <?php echo number_format($rw1['valor3_producto'], 4); ?></option>
                        <?php
}
    ?>
                </select>
...
$update_fact = mysqli_query($conexion, "update facturas_cot set monto_factura='$total_factura' where id_factura='$id_factura'");

?>
<tr>
    <td class='text-right' colspan=5>SUBTOTAL</td>
    <td class='text-right'><b><?php echo $simbolo_moneda . ' ' . number_format($subtotal, 4); ?></b></td>
    <td></td>
</tr>
<tr>
    <td class='text-right' colspan=5><?php echo $nom_impuesto; ?> (<?php echo $impuesto; ?>)% </td>
    <td class='text-right'><?php echo $simbolo_moneda . ' ' . number_format($total_impuesto, 4); ?></td>
    <td></td>
</tr>
<tr>
     <td style="font-size: 20pt;" class='text-right' colspan=5><b>TOTAL <?php echo $simbolo_moneda; ?></b></td>
    <td style="font-size: 20pt; background: #F5B7B1;" class='text-right'><b><?php echo number_format($total_factura, 4); ?></b></td>
    <td></td>
</tr>
</tbody>
</table>
