...
$("#datos_factura").submit(function (event) {
  ...
  if (isNaN(resibido)) {
    $.Notification.notify(
      ...
      "bottom center",
      "NOTIFICACIÓN",
      "EL DATO NO ES VALIDO, INTENTAR DE NUEVO"
    );
    $("#resibido").focus();
    return false;
  }
...
$("#cerrar_caja").submit(function (event) {
  $("#btn_caja").attr("disabled", true);
  var parametros = $(this).serialize();
  $.ajax({
    type: "POST",
    url: "../ajax/cierre_caja.php",
    data: parametros,
    beforeSend: function (objeto) {
      $("#resultados_ajax").html(
        '<img src="../../img/ajax-loader.gif"> Cargando...'
    ...
    success: function (datos) {
      $("#resultados_ajaxx").html(datos);
      $("#btn_caja").attr("disabled", true);
      // $('#cerrarCaja').modal('hide');//CIERRA LA MODAL
      //resetea el formulario
      $("#cerrar_caja")[0].reset();
      //desaparecer la alerta
      $(".alert-success")
